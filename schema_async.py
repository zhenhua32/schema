  ...
  async def validate(self, data):
    try:
      if asyncio.iscoroutinefunction(self._callable):
        return await self._callable(data)
      else:
  ...
  async def validate(self, data):
    ...
    if flavor == DICT:
      ...
      for key, value in data.items():
        for skey in sorted_skeys:
          svalue = s[skey]
          try:
            nkey = await Schema(skey, error=e).validate(key)
          except SchemaError:
            pass
          else:
            if isinstance(skey, Forbidden):
              ...
              # with Optional.
              try:
                nvalue = await Schema(svalue, error=e).validate(value)
              except SchemaError:
                continue
              raise SchemaForbiddenKeyError('Forbidden key encountered: %r in %r' % (nkey, data), e)
            else:
              try:
                nvalue = await Schema(svalue, error=e, ignore_extra_keys=i).validate(value)
              except SchemaError as x:
                k = "Key '%s' error:" % nkey
    ...
    if flavor == TYPE:
      if isinstance(data, s):
        ...
        raise SchemaUnexpectedTypeError('%r should be instance of %r' % (data, s.__name__),
  ...
  def __init__(self, *args, **kwargs):
    ...
    if default is not self._MARKER:
      ...
      if _priority(self._schema) != COMPARABLE:
        raise TypeError('Optional keys with defaults must have simple, '
...
class Const(Schema):
  async def validate(self, data):
    await super(Const, self).validate(data)
    return data
